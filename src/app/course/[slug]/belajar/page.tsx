...
import { CourseLearningPage } from '@/components/features/course-learning-page'
...
/**
 * Generate metadata untuk halaman belajar course
 */
export async function generateMetadata({ params }: CourseLearnPageProps): Promise<Metadata> {
  ...
  return {
    title: `Belajar ${course.title} - ExcelMaster`,
    description: `Belajar ${course.title} - ${course.description}`,
  }
}
 ...
 * Menampilkan konten pembelajaran course dengan video dan materi
 */
export default function CourseLearningPageRoute({ params }: CourseLearnPageProps) {
  const course = mockCourses[params.slug as CourseSlug]
